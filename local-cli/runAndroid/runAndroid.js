...
function tryInstallAppOnDevice(args, device) {
  try {
    const pathToApk = 'JustUnFollow/build/outputs/apk/app-debug.apk';
    const adbPath = getAdbPath();
...
function isSeparateBuildEnabled(gradleFilePath) {
  ...
  const separateBuild = content.match(/enableSeparateBuildPerCPUArchitecture\s+=\s+([\w]+)/);
  return separateBuild && separateBuild.toLowerCase() === 'true';
}
...
function getManifestFile(variant) {
  ...
  // starting the app
  const gradleFilePath = 'JustUnFollow/build.gradle';
  ...
  const paths = ['JustUnFollow/build/intermediates/manifests/full'];
  if (flavor) {
    paths.push(flavor);
  }
